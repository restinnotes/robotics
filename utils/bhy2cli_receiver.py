...
class BHy2CLIReceiver(IMUDataSource):
    ...
    def connect(self, perform_gyro_foc: bool = False, load_bsx_calibration: bool = True) -> bool:
        ...
        # 尝试连接（BSX 校准现在通过链式命令在同一进程中加载）
        success = self._start_process(
            perform_gyro_foc=perform_gyro_foc,
            load_bsx_calibration=load_bsx_calibration
        )
    ...
    def _start_process(self, perform_gyro_foc: bool = False, load_bsx_calibration: bool = True) -> bool:
        """启动 bhy2cli 进程，使用链式命令确保校准数据在同一进程中加载"""
        try:
            cmd = [self.exe_path]
            ...
            current_dir = os.path.dirname(os.path.abspath(__file__))
            project_root = os.path.dirname(current_dir)
            calibration_folder = os.path.join(project_root, "calibration")
            acc_bin = os.path.join(calibration_folder, "acc_calib.bin")
            gyro_bin = os.path.join(calibration_folder, "gyro_calib.bin")

            # 链式命令：先加载 BSX 校准，再启动数据流
            # 这样校准数据在同一进程中加载，不会因为进程结束而丢失
            bsx_loaded = False
            if load_bsx_calibration and os.path.exists(acc_bin) and os.path.exists(gyro_bin):
                print(">>> 检测到 BSX 校准 bin 文件，将通过链式命令加载...")
                # 添加 setbsxparam 命令到链中
                # 格式: setbsxparam 0x201 <acc_file> setbsxparam 0x203 <gyro_file> -c 37:50
                cmd.extend(["setbsxparam", "0x201", acc_bin])
                cmd.extend(["setbsxparam", "0x203", gyro_bin])
                bsx_loaded = True
                print(f"    - 加速度计校准: {acc_bin}")
                print(f"    - 陀螺仪校准: {gyro_bin}")
            ...
            # 等待确认是否有有效数据输出
            # 如果加载了 BSX 校准，可能需要更长时间
            wait_time = 5.0 if bsx_loaded else 3.0
