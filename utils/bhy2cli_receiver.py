...
class BHy2CLIReceiver(IMUDataSource):
    """
    BHy2CLI 接收器，通过子进程运行 bhy2cli.exe 获取传感器数据
    ...
    def __init__(
        ...
        """
        Args:
            exe_path: bhy2cli.exe 的路径 (默认自动查找)
    ...
    def _find_exe(self) -> str:
        """查找 bhy2cli.exe 或 spi_bhy2cli.exe 路径"""
        ...
        raise FileNotFoundError(
            "找不到 bhy2cli 可执行文件。请确保 BHy2CLI 目录下有 spi_bhy2cli.exe 或 bhy2cli.exe"
        )
